...
export class ChatMessageComponent {
  ...
  public get formattedResponse(): SafeHtml | undefined {
    ...
    const foundAmazonSearchRequests: RegExpMatchArray | null = result.match(/".+"/gi);

    foundAmazonSearchRequests?.forEach((searchRequest: string) => {
      result = result.replace(searchRequest, `<a href="${this.createAmazonSearchUrl(searchRequest)}" class="text-blue-300" target="_blank">${searchRequest}</a>`);
    ...
    foundAmazonItems?.forEach((item: string) => {
      ...
      result = result.replace(resultText, `<a href="${this.createAmazonUrl(resultText)}" class="text-blue-300" target="_blank">${resultText}</a>`);
    });
